...
describe('AUTH', () => {
  ...
  test('[3] login', async () => {
    ...
    expect(res.body).toHaveProperty("message", "serkan geri geldi")
  })
...
describe('BİLMECELER', () => {
  ...
  test('[6] get bilmeceler', async () => {
    const payload = { username: 'serkan', "password": "1234" }
    const loginRes = await request(server).post('/api/auth/login').send(payload)
    //expect(loginRes.body.token.username).toBe('serkan')

    const res = await request(server).get('/api/bilmeceler/').set('Authorization', loginRes.body.token)
  ...
  test('[7] post bilmeceler', async () => {
    ...
    const res = await request(server).post('/api/bilmeceler/').set('Authorization', loginRes.body.token).send(bilmecepayload)
